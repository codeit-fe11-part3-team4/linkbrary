...
export const postSignIn = async (email: string, password: string): Promise<AuthSignInResponse> => {
    const response: AxiosResponse<AuthSignInResponse> = await instance.post(PATHS.AUTH + "sign-in", { email, password });
...
// 회원가입하는 API
export const postSignUp = async (email: string, password: string, name: string): Promise<AuthSignUpResponse> => {
  const response: AxiosResponse<AuthSignUpResponse> = await instance.post(PATHS.AUTH + "sign-up", { email, password, name });
  return response.data;
};
...
// sns 회원가입을 위한 API
export const postSignUpWithProvider = async (token: string, redirectUri:string, provider: string): Promise<AuthSignUpResponse> => {
  const response: AxiosResponse<AuthSignUpResponse> = await instance.post(`${PATHS.AUTH}sign-up/${provider}`, { token, redirectUri });
  return response.data;
};
...
export const putFolder = async (folderId: number, name: string): Promise<FolderResponse> => {
  const response: AxiosResponse<FolderResponse> = await instance.put(`${PATHS.FOLDER}${folderId}`, { name });
  return response.data;
};

// Link API
// 특정 폴더에 있는 링크를 가져오는 API
export const getLinksByFolderId = async ( folderId: number, page: number = 1, pageSize: number = 9): Promise<LinkResponse[]> => {
    const response: AxiosResponse<LinkResponse[]> = await instance.get(`${PATHS.FOLDER}${folderId}/links`, {
      params: {
        page,
        pageSize,
      },
...
// 모든 링크를 가져오는 API
export const getLinks = async (page: number = 1, pageSize: number = 9, search: string = ""): Promise<LinkResponse[]> => {
...
// 즐겨찾기된 링크를 가져오는 API
export const getFavorites = async (page: number = 1, pageSize: number = 9): Promise<LinkResponse[]> => {
...
// 특정 링크를 업데이트하는 API
export const putLink = async (linkId: number, data: Partial<LinkResponse>): Promise<LinkResponse> => {
...
// OAuth API
// OAuth 앱을 생성하는 API
export const createOAuthApp = async (appData: { name: string; redirectUri: string }): Promise<OAuthAppResponse> => {
...
export const checkEmail = async (email: string): Promise<boolean> => {
  const response: AxiosResponse<{ exists: boolean }> = await instance.post(PATHS.USER + "/check-email", { email });
